...
class MothPost(View):
    def get(self, request, year, month):
        text = ''
        ...  # формируем стати за год и месяц
        return HttpResponse(f'Posts from {month}/{year}<br>{text}')


def post_detail(request, year, moth, slug):
    ...  # формируем статьи за год и месяц по индентификаторую
    ...
    post = {
        ...
        'content': 'В процессе написания очередной программы задумлся на тем, '
...
def my_view(request):
    context = {'name': "LA' BUSURrr",
               'tag': "LA ' SEX MACHIN INSTRUCTOR"}
